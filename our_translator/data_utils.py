...
# URLs for WMT data.
TRAIN_URL = "http://www.statmt.org/europarl/v7/es-en.tgz"
DEV_URL = "http://www.statmt.org/wmt07/devsets.tgz"


def maybe_download(directory, filename, url):
  """Download filename from url unless it's already in directory. The folder is in C:"""
...
def get_wmt_enfr_train_set(directory):
  """Download the en-es training corpus to directory unless it's there."""
  train_path = os.path.join(directory, "europarl-v7.es-en")
  if not (gfile.Exists(train_path +".es") and gfile.Exists(train_path +".en")): 
    corpus_file = maybe_download(directory, "en-es.tgz", TRAIN_URL)
    print("Extracting tar file %s" % corpus_file)
    corpus_tar = tarfile.open(corpus_file, "r")
    corpus_tar.extractall(directory)
  return train_path


def get_wmt_enfr_dev_set(directory):
  """Download the en-es test corpus to directory unless it's there."""
  dev_name = "devtest2006"
  dev_path = os.path.join(directory, dev_name)
  if not (gfile.Exists(dev_path + ".es") and gfile.Exists(dev_path + ".en")):
    dev_file = maybe_download(directory, "devsets.tgz.gz", DEV_URL)
    print("Extracting tgz file %s" % dev_file)

    dev_tar = tarfile.open(dev_file, "r")
    dev_tar.extractall(directory)
    current = os.path.join(directory, "wmt07\\devtest", dev_name)
    root = os.path.join(directory, dev_name)
    os.rename(current + ".es", root + ".es")
    os.rename(current + ".en", root + ".en")
  return dev_path
